...
def load_config():
    ...
    if not os.path.exists(CONFIG_FILE):
        ...
        print(
            "Please copy embed_config.json.example to embed_config.json and fill in your settings.")
        sys.exit(1)
...
def generate_embed_url(config):
    ...
    external_user_id = "broxel_exec_1"
    first_name = "Broxel"
    last_name = "Executive"
...
def render_page(title, active_tab, content_html, config, embed_url="",
    mode="private"):
    mode_badge = "Looker Enterprise • Private Embed" if mode == "private" else "Looker Embed Edition • Signed SSO"
    ...
    nav_items = [
        ...
        ("PLATFORM & ARCHITECTURE", [
            ("/about", "Looker Integration", "fa-layer-group"),
...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; font-family: 'Figtree', 'Open Sans', sans-serif; }}
        ...
        /* Looker Iframe */
        .iframe-wrapper {{ width: 100%; height: 850px; border-radius: 10px; overflow: hidden; border: 1px solid var(--border-color); background-color: #f8fafc; position: relative; }}
...
<body>
        ...
        <div class="sidebar-footer">
            <div class="user-widget">
                ...
                <div class="user-info">
                    ...
                    <div class="user-role">Fintech Analytics Tier</div>
                </div>
            </div>
        </div>
    </aside>
    ...
    <div class="main-wrapper">
        ...
        <footer class="footer">
            © 2026 Broxel Fintech • Más que una tarjeta, un infinito de posibilidades. • Looker Google Cloud Enterprise
        </footer>
    </div>
</body>
</html>"""
...
class EmbedHandler(BaseHTTPRequestHandler):
    """HTTP Handler serving a Broxel-branded SaaS analytics portal application."""
        ...
        try:
            ...
            if self.path == "/" or self.path == "/dashboard":
                ...
                <div class="page-header">
                    <div>
                        <h1 class="page-title">Broxel Executive Analytics</h1>
                        <p class="page-subtitle">Monitoreo en tiempo real de transacciones, cuentas duales (MXN/USD) y rendimiento e-commerce.</p>
                    </div>
                    <div>
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-title"><i class="fa-solid fa-chart-pie" style="color: #0075ff;"></i> Looker Enterprise Embedded Dashboard</div>
                ...
                """
                html = render_page("Dashboard", "Executive Dashboard",
                                   content_html, config, embed_url, mode)
                ...
                """
                html = render_page("Reportes", "Financial & MXN Reports",
                                   content_html, config, embed_url, mode)
                self.send_response(200)
                self.send_header("Content-type", "text/html")
                self.end_headers()
                ...
                <div class="page-header">
                    <div>
                        <h1 class="page-title">Arquitectura de Integración Looker</h1>
                        <p class="page-subtitle">Especificación técnica de la integración Looker Google Cloud Enterprise para Broxel Fintech.</p>
                    </div>
                </div>

                <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 2rem;">
                    <div class="card" style="margin-bottom: 0;">
                        <div class="card-header"><div class="card-title"><i class="fa-solid fa-network-wired" style="color: #0075ff;"></i> Arquitectura Looker Enterprise Private Embed</div></div>
                        <div style="line-height: 1.8; color: #475569; font-size: 0.95rem;">
                            <p style="margin-bottom: 1.25rem;">Este portal implementa <strong>Looker Google Cloud Enterprise Edition Private Embedding</strong>, optimizado específicamente para los estándares de seguridad bancaria y fintech de <strong>Broxel</strong> sin exponer claves secretas de administración en el frontend.</p>
                            
                            <h3 style="color: #0f172a; font-size: 1.1rem; margin: 1.5rem 0 0.75rem;">Flujo de Autenticación Segura:</h3>
                            <div style="display: flex; flex-direction: column; gap: 1rem;">
                                <div style="display: flex; gap: 1rem; align-items: flex-start; background: #f4f7fa; padding: 1rem; border-radius: 8px; border: 1px solid var(--border-color);">
                                    ...
                                    <div><strong>Sesión Nativa Google Workspace / Looker:</strong> El iframe verifica de forma transparente la sesión activa del directivo de Broxel en Google Cloud Identity o Looker.</div>
                                </div>
                                <div style="display: flex; gap: 1rem; align-items: flex-start; background: #f4f7fa; padding: 1rem; border-radius: 8px; border: 1px solid var(--border-color);">
                                    ...
                                    <div><strong>Login Interactivo (allow_login_screen):</strong> En caso de expiración de sesión, Looker renderiza su pantalla de autenticación nativa dentro del iframe sin generar errores 401.</div>
                                </div>
                                <div style="display: flex; gap: 1rem; align-items: flex-start; background: #f4f7fa; padding: 1rem; border-radius: 8px; border: 1px solid var(--border-color);">
                                    ...
                                    <div><strong>Cero Exposición de Secretos:</strong> Al utilizar Private Embedding con Enterprise Edition, no es necesario gestionar secretos HMAC ni llaves de Embed Edition en los servidores web.</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="card" style="margin-bottom: 1.5rem;">
                            <div class="card-header"><div class="card-title"><i class="fa-solid fa-shield-halved" style="color: #00c2b2;"></i> Estado de Conexión</div></div>
                            <div style="display: flex; flex-direction: column; gap: 1rem; font-size: 0.875rem;">
                                <div style="display: flex; justify-content: space-between;"><span>Looker API v4.0:</span> <strong style="color: #008075;">● Conectado (SSL)</strong></div>
                                <div style="display: flex; justify-content: space-between;"><span>BigQuery Warehouse:</span> <strong style="color: #008075;">● Activo</strong></div>
                                <div style="display: flex; justify-content: space-between;"><span>Latencia de Sesión:</span> <strong>18 ms</strong></div>
                ...
                """
                html = render_page("Arquitectura", "Looker Integration",
                                   content_html, config, embed_url, mode)
                self.send_response(200)
                self.send_header("Content-type", "text/html")
                self.end_headers()
                ...
                """
                html = render_page("Preferencias", "Account Preferences",
                                   content_html, config, embed_url, mode)
                self.send_response(200)
                self.send_header("Content-type", "text/html")
                self.end_headers()
